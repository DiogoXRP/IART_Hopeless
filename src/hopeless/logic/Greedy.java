...
public class Greedy {
	...
	private Board last;
	private static GreedyBoardComparator comp = new GreedyBoardComparator(); // in
																				// case
																				// prio_queue
					...
					for (int jj = slice - z2; jj >= z1; jj--) {
						...
						if (current_board.getBoard()[jj][slice - jj] != ' ') {
							...
							if (!current_board.getVisited().contains(new Position(jj, slice - jj))) {
								...
								test_boards.get(indexx).professionalalgorithm(
										test_boards.get(indexx).getBoard()[jj][slice - jj], jj, slice - jj);
